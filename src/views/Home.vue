  ...
  <div class="flex justify-between">
    ...
    <div class="w-3/4">
      ...
      <div v-else id="empty-conversation" class="flex flex-col w-full">
        ...
        <div v-if="gallery" id="conversation-container" class="flex flex-row flex-wrap mt-24 text-center flex-1 overflow-y-auto shadow-inner p-5 space-x-2 space-y-2">
          <div v-for="(messages, indexMessage) in imageList" v-bind:key="indexMessage">
            <img v-for="(item, index) in messages.images" v-bind:key="index" :imagename="`${item.name}`" :src="item.blob" alt="404 BILD NOT FOUND" loading="lazy" class="galleryimage">
          </div>
        </div>
      </div>
    </div>
